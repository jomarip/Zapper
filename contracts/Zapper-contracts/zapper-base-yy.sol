...
import "../interfaces/yyvault.sol";
import "../interfaces/uniAmm.sol";

abstract contract ZapperBaseYY {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    using SafeERC20 for IYY;
    ...
    function _swapAndStake(address yyVault, uint256 tokenAmountOutMin, address tokenIn) public virtual;

    function zapInAVAX(address yyVault, uint256 tokenAmountOutMin, address tokenIn) external payable{
        ...
        if (tokenIn != wavax){
            ...
            _swapAndStake(yyVault, tokenAmountOutMin, tokenIn);
        }else{
            _swapAndStake(yyVault, tokenAmountOutMin, tokenIn);
        }
    }
    ...
    function zapIn(address yyVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount) external {
    ...
    function zapOutAndSwap(address yyVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) public virtual;
    ...
    function _getVaultPair(address yyVault) internal view returns (IYY vault, IUniPair pair){

        vault = IYY(yyVault);
        pair = IUniPair(vault.depositToken());
    ...
    function zapOut(address yyVault, uint256 withdrawAmount) external {
        (IYY vault, IUniPair pair) = _getVaultPair(yyVault);

        IERC20(yyVault).safeTransferFrom(msg.sender, address(this), withdrawAmount);
        vault.withdraw(withdrawAmount);

        uint256 _amt = IERC20(yyVault).balanceOf(address(this));

        //take dev fee and transfer
        uint256 _feeDev = _amt.mul(useDevFee).div(useDevMax);
        IERC20(yyVault).safeTransfer(devFund, _feeDev);

        //take treasury fee and transfer
        uint256 _feeTreasury = _amt.mul(useTreasuryFee).div(useTreasuryMax);
        IERC20(yyVault).safeTransfer(treasury,_feeTreasury);
