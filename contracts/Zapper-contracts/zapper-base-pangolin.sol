...
abstract contract ZapperBasePangolin {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    using SafeERC20 for IMiniChef;
    ...
    function _swapAndStake(uint256 pid, uint256 tokenAmountOutMin, address tokenIn) public virtual;

    function zapInAVAX(uint256 pid, uint256 tokenAmountOutMin, address tokenIn) external payable{
    ...
    function zapOutAndSwap(uint256 pid, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) public virtual;
    ...
    function _getMiniChefPair(uint256 pid) internal view returns (IMiniChef vault, IUniPair pair){

                
        vault = IMiniChef(miniChef);
        address lpToken_addr = vault.lpToken(pid);
        pair = IUniPair(lpToken_addr);
    ...
    //zapOut starts with the liquidity token that has alrady been withdrawn from the Farm
    function zapOut(uint256 pid, uint256 withdrawAmount) external {
        ...
        uint256 _amt = IERC20(miniChef).balanceOf(address(this));

        //take dev fee and transfer
        uint256 _feeDev = _amt.mul(useDevFee).div(useDevMax);
        IERC20(miniChef).safeTransfer(devFund, _feeDev);

        //take treasury fee and transfer
        uint256 _feeTreasury = _amt.mul(useTreasuryFee).div(useTreasuryMax);
        IERC20(miniChef).safeTransfer(treasury,_feeTreasury);
